...
const char *malloc_conf = "narenas:16";
...
int nthreads = 4;
gboolean debug, foreground, verbose, mts;
...
static GOptionEntry entries[] = {
    ...
    {"allow-mts", 0, 0, G_OPTION_ARG_NONE, &mts,
     "Allow to run with MultiThreadedSlave on", NULL},
...
int main(int ac, char **av)
{
    ...
    for (;;) {
        if (!mts && slave_parallel(slave)) {
            slave_sleep(slave, 60, "What if... not parallel anymore?");
            continue;
        }
